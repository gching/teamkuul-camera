...
@property (nonatomic, weak) CloudRecoViewController * rootViewController;
...
- (BOOL)loadRemoteMediaFromURL:(NSURL*)url;
...
//------------------------------------------------------------------------------
#pragma mark - Lifecycle
- (id)initWithRootViewController:(CloudRecoViewController *) viewController
{
    self = [super init];
...
//    (void)AudioSessionSetActive(true);
    ...
    NSLog(@"Loading something with name %@", filename);
    ...
    if (NOT_READY != mediaState && ERROR != mediaState) {
        ...
        if (NSNotFound == [filename rangeOfString:@"://"].location) {
            ...
            NSLog(@"The media url is %@", mediaURL);

            ret = [self loadRemoteMediaFromURL:mediaURL];
        }
    }
             ...
             dispatch_async(dispatch_get_main_queue(),
                            ^{
                                ...
                                if (status == AVKeyValueStatusLoaded) {
                                    ...
                                    // Error
                                    NSLog(@"Error - The asset's tracks were not loaded: %@ %@", [error localizedDescription],  url);
                                    mediaState = ERROR;
                                }
                            });
         }];
