...
- (id)initWithFrame:(CGRect)frame rootViewController:(CloudRecoViewController *) rootViewController appSession:(SampleApplicationSession *) app
{
...
/*
        ...
        if (ERROR != mediaState && NOT_READY != mediaState && PLAYING != mediaState) {
            ...
            NSLog(@"Touched target is %d", touchedTarget);
...
- (void)renderFrameQCAR
{
    ...
    for (int i = 0; i < numActiveTrackables; ++i) {
        ...
        NSLog(@"The image target name is %s", imageTarget.getName());
        NSLog(@"The image meta data is %s", imageTarget.getMetaData());

        // We check to see if the new imageTarget is not equal to the currently
        // playing one, so we reset the player.
        if (strcmp([videoData[playerIndex].targetName UTF8String], imageTarget.getName()) != 0 ){
          [videoPlayerHelper[playerIndex] unload];
          videoPlaybackTime[playerIndex] = VIDEO_PLAYBACK_CURRENT_POSITION;
          if (NO == [videoPlayerHelper[playerIndex] load:[NSString stringWithUTF8String: imageTarget.getMetaData()] playImmediately:NO fromPosition:videoPlaybackTime[playerIndex]]) {
              NSLog(@"Failed to load media");
          }
            videoData[playerIndex].targetName = [NSString stringWithUTF8String: imageTarget.getName()];
        }
